    ...
    <div className="min-h-screen bg-background text-foreground pb-20">
      ...
      <header className="border-b border-foreground/20 py-16 pt-32">
        <div className="max-w-4xl mx-auto px-8 text-center">
          <h1 className="text-6xl font-bold mb-4">{t('home.title', 'Constant Suchet')}</h1>
          <p className="text-2xl text-foreground/80 mb-6">{t('home.subtitle', 'Full Stack Developer')}</p>
          <p className="mt-6 text-lg text-foreground/70 max-w-3xl mx-auto leading-relaxed">
            {t('home.description', 'Passionate about creating clean, efficient, and user-friendly web applications. I enjoy solving complex problems and turning ideas into reality through code.')}
          </p>
        </div>
      </header>
      ...
      <section id="projects" className="py-16">
        <div className="max-w-4xl mx-auto px-6">
          <h2 className="text-3xl font-bold mb-12 text-center">{t('home.projectsTitle', 'Projects')}</h2>
          <div className="grid gap-8 md:grid-cols-2">
            {projects.map((project, index) => (
              <Link href={project.href} key={index}>
                <div className="border border-foreground/20 p-6 hover:border-foreground/40 transition-colors h-full">
                  <h3 className="text-xl font-semibold mb-3">{t(`projects.${project.key}.title`, project.title)}</h3>
                  <p className="text-foreground/80 mb-4 leading-relaxed">{t(`projects.${project.key}.description`, project.description)}</p>
      ...
      <section id="skills" className="py-16 border-t border-foreground/20">
        <div className="max-w-4xl mx-auto px-6">
          <h2 className="text-3xl font-bold mb-12 text-center">{t('home.skillsTitle', 'Skills')}</h2>
          <div className="grid gap-8 md:grid-cols-3">
            {Object.entries(skills).map(([category, skillList]) => (
              <div key={category} className="text-center">
                <h3 className="text-xl font-semibold mb-6 border-b border-foreground/20 pb-2">{t(`skills.${category.toLowerCase().replace(/\s+&\s+/g, '').replace(/\s+/g, '')}`, category)}</h3>
      ...
      <section id="contact" className="py-16 border-t border-foreground/20">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <h2 className="text-3xl font-bold mb-8">{t('nav.contact', 'Get In Touch')}</h2>
          <p className="text-foreground/80 mb-8 max-w-2xl mx-auto">
             {t('contact.description', "I'm always interested in new opportunities and collaborations. Feel free to reach out if you'd like to work together.")}
           </p>
      ...
      <footer className="border-t border-foreground/20 py-8">
        <div className="max-w-4xl mx-auto px-6 text-center text-foreground/60">
          <p>{t('footer.copyright', '© 2024 Constant Suchet. All rights reserved.')}</p>
        </div>
      </footer>
    </div>
  );
